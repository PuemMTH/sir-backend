...
[
  {
    ...
    "thumbnail_r2_key": "assets/xyz789/abc123.preview",
...
Upload a new asset file. Content is stored in R2; metadata (original filename, MIME type, size, optional thumbnail key) is stored in D1. Maximum file size is 10 MB.
...
| `thumbnail` | No | Small preview image generated by the client (max 1 MB) |
...
Fetch the small preview image for an uploaded asset.

**Response** `200 OK` — image binary

| Header | Values |
|--------|--------|
| `X-Cache` | `CF-HIT` or `MISS` |
...
Fetch the original uploaded asset content.

**Response** `200 OK` — original binary content
...
Delete the asset and thumbnail from R2 (best-effort) and remove the D1 row.
...
| GET | `/api/assets/:id/preview` | JWT | Fetch an asset preview image |
| GET | `/api/assets/:id/content` | JWT | Fetch original asset content |
